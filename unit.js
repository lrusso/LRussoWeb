...
describe("Llama/index.html", function () {
  it("Should exist", function () {
    expect(fileExists("Llama/index.html")).toBe(true)
  })

  it("Should not have changed", function () {
    const expectedHash = "7a4cc1f8"
    let currentHash = ""

    try {
      const content = readFileSync("Llama/index.html", "utf8")
      currentHash = generateHash(content)
    } catch (err) {
      //
    }
  ...
  it("Should define the i18n variable", function () {
    const variableExists = getVariable("Llama/index.html", "APP_STRINGS")
    expect(variableExists).not.toBe("")
  })

  it("Should have equal i18n key counts across languages", function () {
    const APP_STRINGS = getVariable("Llama/index.html", "APP_STRINGS")
      .replace(/,\s*([}\]])/g, "$1")
      // convert hex escapes
      .replace(/\\x([0-9A-Fa-f]{2})/g, function (_, hex) {
        return String.fromCharCode(parseInt(hex, 16))
