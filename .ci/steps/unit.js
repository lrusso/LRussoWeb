...
// ----------------------------------------------------------------------------
//
// add a jest test to the bottom of this file, that must use:
//
// describe, it, expect
//
// then run:
...
describe("CSSMinifier/index.html", function () {
  it("Should exist", function () {
    expect(fileExists("CSSMinifier/index.html")).toBe(true)
  })

  it("Should not have changed", function () {
    const expectedHash = "77233846"
    let currentHash = ""

    try {
      const content = readFileSync("CSSMinifier/index.html", "utf8")
  ...
  it("Should define the i18n variable", function () {
    const variableExists = getVariable("CSSMinifier/index.html", "APP_STRINGS")
    expect(variableExists).not.toBe("")
  })

  it("Should have equal i18n key counts across languages", function () {
    const APP_STRINGS = getVariable("CSSMinifier/index.html", "APP_STRINGS")
      .replace(/,\s*([}\]])/g, "$1")
      // convert hex escapes
...
describe("GitHubRead/index.html", function () {
  it("Should exist", function () {
    expect(fileExists("GitHubRead/index.html")).toBe(true)
  })

  it("Should not have changed", function () {
    const expectedHash = "54521c00"
    let currentHash = ""

    try {
      const content = readFileSync("GitHubRead/index.html", "utf8")
      currentHash = generateHash(content)
    } catch (err) {
      //
    }
  ...
  it("Should define the i18n variable", function () {
    const variableExists = getVariable("GitHubRead/index.html", "APP_STRINGS")
    expect(variableExists).not.toBe("")
  })

  it("Should have equal i18n key counts across languages", function () {
    const APP_STRINGS = getVariable("GitHubRead/index.html", "APP_STRINGS")
      .replace(/,\s*([}\]])/g, "$1")
      // convert hex escapes
      .replace(/\\x([0-9A-Fa-f]{2})/g, function (_, hex) {
        return String.fromCharCode(parseInt(hex, 16))
...
describe("JavaScriptMinifier/index.html", function () {
  it("Should exist", function () {
    expect(fileExists("JavaScriptMinifier/index.html")).toBe(true)
  })

  it("Should not have changed", function () {
    const expectedHash = "10807726"
    let currentHash = ""

    try {
      const content = readFileSync("JavaScriptMinifier/index.html", "utf8")
      currentHash = generateHash(content)
    } catch (err) {
      //
    }
  ...
  it("Should define the i18n variable", function () {
    const variableExists = getVariable(
      "JavaScriptMinifier/index.html",
      "APP_STRINGS"
    )
    expect(variableExists).not.toBe("")
  })
  ...
  it("Should have equal i18n key counts across languages", function () {
    const APP_STRINGS = getVariable("JavaScriptMinifier/index.html", "APP_STRINGS")
      .replace(/,\s*([}\]])/g, "$1")
      // convert hex escapes
      .replace(/\\x([0-9A-Fa-f]{2})/g, function (_, hex) {
        return String.fromCharCode(parseInt(hex, 16))
...
describe("MP3TagEditor/index.html", function () {
  ...
  it("Should not have changed", function () {
    const expectedHash = "45489f4c"
    let currentHash = ""

    try {
      const content = readFileSync("MP3TagEditor/index.html", "utf8")
      currentHash = generateHash(content)
    } catch (err) {
      //
    }
  ...
  it("Should define the i18n variable", function () {
    const variableExists = getVariable("MP3TagEditor/index.html", "APP_STRINGS")
    expect(variableExists).not.toBe("")
  })

  it("Should have equal i18n key counts across languages", function () {
    const APP_STRINGS = getVariable("MP3TagEditor/index.html", "APP_STRINGS")
      .replace(/,\s*([}\]])/g, "$1")
      // convert hex escapes
      .replace(/\\x([0-9A-Fa-f]{2})/g, function (_, hex) {
        return String.fromCharCode(parseInt(hex, 16))
...
describe("PDFConverter/index.html", function () {
  it("Should exist", function () {
    expect(fileExists("PDFConverter/index.html")).toBe(true)
  })

  it("Should not have changed", function () {
    const expectedHash = "546c017c"
    let currentHash = ""

    try {
      const content = readFileSync("PDFConverter/index.html", "utf8")
      currentHash = generateHash(content)
    } catch (err) {
      //
    }
  ...
  it("Should define the i18n variable", function () {
    const variableExists = getVariable("PDFConverter/index.html", "APP_STRINGS")
    expect(variableExists).not.toBe("")
  })

  it("Should have equal i18n key counts across languages", function () {
    const APP_STRINGS = getVariable("PDFConverter/index.html", "APP_STRINGS")
      .replace(/,\s*([}\]])/g, "$1")
      // convert hex escapes
      .replace(/\\x([0-9A-Fa-f]{2})/g, function (_, hex) {
        return String.fromCharCode(parseInt(hex, 16))
...
describe("PuzzleBobble/index.html", function () {
  it("Should exist", function () {
    expect(fileExists("PuzzleBobble/index.html")).toBe(true)
  })

  it("Should not have changed", function () {
    const expectedHash = "746180b7"
    let currentHash = ""

    try {
      const content = readFileSync("PuzzleBobble/index.html", "utf8")
      currentHash = generateHash(content)
    } catch (err) {
      //
    }
  ...
  it("Should define the i18n variable", function () {
    const variableExists = getVariable("PuzzleBobble/index.html", "APP_STRINGS")
    expect(variableExists).not.toBe("")
  })

  it("Should have equal i18n key counts across languages", function () {
    const APP_STRINGS = getVariable("PuzzleBobble/index.html", "APP_STRINGS")
      .replace(/,\s*([}\]])/g, "$1")
      // convert hex escapes
      .replace(/\\x([0-9A-Fa-f]{2})/g, function (_, hex) {
        return String.fromCharCode(parseInt(hex, 16))
...
describe("Resources/index.html", function () {
  ...
  it("Should not use the fill property in SVG tags", function () {
    let svgsWithFill = []

    try {
      const content = readFileSync("Resources/index.html", "utf8")
      const svgOpenTagRegex = /<svg\b[^>]*>/gi
      const svgOpenTags = content.match(svgOpenTagRegex) || []
      svgOpenTags.forEach(function (tag) {
        if (/\bfill\s*=/i.test(tag)) {
          const idMatch = tag.match(/\bid\s*=\s*["']([^"']+)["']/i)
          svgsWithFill.push(idMatch ? idMatch[1] : tag)
        }
      })
    } catch (err) {
      svgsWithFill.push("Error reading file")
    }

    if (svgsWithFill.length > 0) {
      throw new Error("SVG tags using fill: " + svgsWithFill.join(", "))
    }

    expect(svgsWithFill.length).toBe(0)
  })

  it("Should use the xmlns property in SVG tags", function () {
    let svgsWithXMLNS = []
    ...
    try {
      ...
      svgOpenTags.forEach(function (tag) {
        if (!/\bxmlns\s*=/i.test(tag)) {
          const idMatch = tag.match(/\bid\s*=\s*["']([^"']+)["']/i)
          svgsWithXMLNS.push(idMatch ? idMatch[1] : tag)
        }
      })
    } catch (err) {
      svgsWithXMLNS.push("Error reading file")
    }

    if (svgsWithXMLNS.length > 0) {
      throw new Error("SVG tags not using xmlns: " + svgsWithXMLNS.join(", "))
    }

    expect(svgsWithXMLNS.length).toBe(0)
  })

  it("Should not have duplicated ID attributes", function () {
    let duplicatedIDs = false
    ...
    try {
      ...
      while ((match = regex.exec(content)) !== null) {
        const id = match[1]
        if (seen.has(id)) {
          if (typeof duplicatedIDs === "boolean") {
            duplicatedIDs = []
          }
    ...
    expect(duplicatedIDs).toEqual(false)
  })

  it("Should not have duplicated ALT attributes", function () {
    let duplicatedALTs = false
    ...
    try {
      ...
      while ((match = regex.exec(content)) !== null) {
        ...
        if (seen.has(altLower)) {
          if (typeof duplicatedALTs === "boolean") {
            duplicatedALTs = []
          }
          duplicatedALTs.push(alt)
        } else {
          seen.add(altLower)
        }
      }
    ...
    expect(duplicatedALTs).toEqual(false)
  })
...
describe("SignatureGenerator/index.html", function () {
  it("Should exist", function () {
    expect(fileExists("SignatureGenerator/index.html")).toBe(true)
  })

  it("Should not have changed", function () {
    const expectedHash = "1440dea2"
    let currentHash = ""

    try {
      const content = readFileSync("SignatureGenerator/index.html", "utf8")
      currentHash = generateHash(content)
    } catch (err) {
      //
    }
  ...
  it("Should have equal i18n key counts across languages", function () {
    const APP_STRINGS = getVariable("SignatureGenerator/index.html", "APP_STRINGS")
      .replace(/,\s*([}\]])/g, "$1")
      // convert hex escapes
      .replace(/\\x([0-9A-Fa-f]{2})/g, function (_, hex) {
        return String.fromCharCode(parseInt(hex, 16))
...
describe("VideoCallWallpaper/index.html", function () {
  it("Should exist", function () {
    expect(fileExists("VideoCallWallpaper/index.html")).toBe(true)
  })

  it("Should not have changed", function () {
    const expectedHash = "26aef0b0"
    let currentHash = ""

    try {
      const content = readFileSync("VideoCallWallpaper/index.html", "utf8")
      currentHash = generateHash(content)
    } catch (err) {
      //
    }
  ...
  it("Should have equal i18n key counts across languages", function () {
    const APP_STRINGS = getVariable("VideoCallWallpaper/index.html", "APP_STRINGS")
      .replace(/,\s*([}\]])/g, "$1")
      // convert hex escapes
      .replace(/\\x([0-9A-Fa-f]{2})/g, function (_, hex) {
        return String.fromCharCode(parseInt(hex, 16))
...
describe("WebZip/index.html", function () {
  it("Should exist", function () {
    expect(fileExists("WebZip/index.html")).toBe(true)
  })

  it("Should not have changed", function () {
    const expectedHash = "60ce43d6"
    let currentHash = ""

    try {
      const content = readFileSync("WebZip/index.html", "utf8")
      currentHash = generateHash(content)
    } catch (err) {
      //
    }
  ...
  it("Should define the i18n variable", function () {
    const variableExists = getVariable("WebZip/index.html", "APP_STRINGS")
    expect(variableExists).not.toBe("")
  })

  it("Should have equal i18n key counts across languages", function () {
    const APP_STRINGS = getVariable("WebZip/index.html", "APP_STRINGS")
      .replace(/,\s*([}\]])/g, "$1")
      // convert hex escapes
      .replace(/\\x([0-9A-Fa-f]{2})/g, function (_, hex) {
        return String.fromCharCode(parseInt(hex, 16))
...
describe(".ci/steps/e2e.jar", function () {
  it("Should exist", function () {
    expect(fileExists(".ci/steps/e2e.jar")).toBe(true)
  })

  it("Should not have changed", function () {
    const expectedHash = "2299d341"
    let currentHash = ""

    try {
      const content = readFileSync(".ci/steps/e2e.jar", "utf8")
      currentHash = generateHash(content)
    } catch (err) {
      //
    }
...
describe(".ci/steps/e2e.js", function () {
  it("Should exist", function () {
    expect(fileExists(".ci/steps/e2e.js")).toBe(true)
  })

  it("Should not have changed", function () {
    const expectedHash = "46cd3910"
    let currentHash = ""

    try {
      const content = readFileSync(".ci/steps/e2e.js", "utf8")
      currentHash = generateHash(content)
    } catch (err) {
      //
    }
...
describe(".ci/steps/eslint.js", function () {
  it("Should exist", function () {
    expect(fileExists(".ci/steps/eslint.js")).toBe(true)
  })

  it("Should not have changed", function () {
    const expectedHash = "16e4fb5d"
    let currentHash = ""

    try {
      const content = readFileSync(".ci/steps/eslint.js", "utf8")
      currentHash = generateHash(content)
    } catch (err) {
      //
    }
...
describe(".ci/steps/prettier.js", function () {
  it("Should exist", function () {
    expect(fileExists(".ci/steps/prettier.js")).toBe(true)
  })

  it("Should not have changed", function () {
    const expectedHash = "29a42868"
    let currentHash = ""

    try {
      const content = readFileSync(".ci/steps/prettier.js", "utf8")
      currentHash = generateHash(content)
    } catch (err) {
      //
    }
...
describe(".ci/steps/w3c.jar", function () {
  it("Should exist", function () {
    expect(fileExists(".ci/steps/w3c.jar")).toBe(true)
  })
  ...
  it("Should not have changed", function () {
    ...
    try {
      const content = readFileSync(".ci/steps/w3c.jar", "utf8")
      currentHash = generateHash(content)
    } catch (err) {
      //
    }
...
describe(".ci/steps/w3c.js", function () {
  it("Should exist", function () {
    expect(fileExists(".ci/steps/w3c.js")).toBe(true)
  })

  it("Should not have changed", function () {
    const expectedHash = "8dcbc12"
    let currentHash = ""

    try {
      const content = readFileSync(".ci/steps/w3c.js", "utf8")
      currentHash = generateHash(content)
    } catch (err) {
      //
    }
...
describe("No inline styles", function () {
  ...
  const exceptionFiles = ["SignatureGenerator/index.html"]
  const filesToCheck = allFiles.filter(function (file) {
    return (
      targetExts.includes(extname(file)) &&
      !exceptionFiles.some(function (ex) {
        return file.endsWith(ex)
      })
    )
  })
