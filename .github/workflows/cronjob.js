...
const cmd = (command) => execSync(command, { maxBuffer: 10000000 }).toString()
...
// READING NEWSPAPERS AND SENDING AN EMAIL
const task5 = (newspapers) => {
  let result = ""

  const h2Regex = /<h2[^>]*>(.*?)<\/h2>/gim
  const tagRegex = /<a[^>]*>|<\/a>|<span[^>]*>|<\/span>/gim

  newspapers.forEach((section) => {
    try {
      for (const [sectionTitle, sections] of Object.entries(section)) {
        ...
        sections.forEach((sectionURL) => {
          const htmlString = curl(sectionURL)
          ...
          while ((match = h2Regex.exec(htmlString))) {
            ...
            const sectionPath = sectionURL.substring(sectionURL.lastIndexOf("/") + 1)
            const isInternalLink = match[1].includes("href") && !match[1].includes(sectionPath)
            const hasMinimumWords = textWithoutTags.split(/\s+/).filter(Boolean).length > 4
            const notExceedsLimit = counter < maxHeadlines
            const shouldBeIncluded = hasMinimumWords && notExceedsLimit && !isInternalLink

            if (shouldBeIncluded) {
              result =
                result +
                "&#8226; " +
                (textWithoutTags.endsWith(".") ? textWithoutTags : textWithoutTags) +
                ".<br>"
              counter = counter + 1
            }
          }
  ...
  // CHECKING IF AN EMAIL CAN BE SENT
  if (process.env.GOOGLE_API_KEY) {
    try {
      const subject = "Listado de titulares"
      const url =
      ...
      execSync('curl -X POST -d "' + postData + '" ' + url)
    } catch (err) {
      //
    }
  }
...
const runTasks = () => {
  task1(settings.W3C_CHECK)
  task2(settings.URLS_CHECK)
  task3(settings.DATES_CHECK, new Date())
  task4(settings.DOMAINS_CHECK, new Date())
  task5(settings.NEWSPAPERS_CHECK)
